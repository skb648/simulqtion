...
class SensorCollector(context: Context) : SensorEventListener {
    ...
    fun snapshot(cameraTimestampNs: Long): ImuSnapshot {
        ...
        return ImuSnapshot(cameraTimestampNs, near(accel), near(gyro), near(magnet), near(rotation))
    }
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        when (event.sensor.type) {
            ...
            Sensor.TYPE_ROTATION_VECTOR -> rotation = event.timestamp to value
        }
    }
