...
class MotorApi(private val baseUrl: String) {
    ...
    fun analyze(files: List<File>): String {
        ...
        val conn = (URL("$baseUrl/v1/pipeline/dc-motor").openConnection() as HttpURLConnection).apply { requestMethod='POST'; doOutput=true; connectTimeout=5000; readTimeout=30000; setRequestProperty('Content-Type', "multipart/form-data; boundary=$boundary") }
        conn.outputStream.use { out ->
            for ((index, file) in files.withIndex()) {
                out.write(("--$boundary\r\nContent-Disposition: form-data; name=\"files\"; filename=\"view_$index.jpg\"\r\nContent-Type: image/jpeg\r\n\r\n").toByteArray())
                file.inputStream().use { it.copyTo(out) }; out.write("\r\n".toByteArray())
            }
            out.write("--$boundary--\r\n".toByteArray())
        }
    ...
    fun simulate(voltage: Float): Simulation {
        ...
        val json = postJson('/v1/simulations/dc-motor', body); val final = json.getJSONObject('final')
    ...
    private fun postJson(path: String, json: String): JSONObject {
        val conn = (URL("$baseUrl$path").openConnection() as HttpURLConnection).apply { requestMethod='POST'; doOutput=true; connectTimeout=5000; readTimeout=30000; setRequestProperty('Content-Type','application/json') }
